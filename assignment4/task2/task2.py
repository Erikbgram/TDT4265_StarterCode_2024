...
def calculate_iou(prediction_box, gt_box):
    ...
    """
    # YOUR CODE HERE
    w_a = prediction_box[2]-prediction_box[0] #width of box a
    h_a = prediction_box[3]-prediction_box[1] #height of box a
    w_b = gt_box[2]-gt_box[0]                   #width of box b
    h_b = gt_box[3]-gt_box[1]                   #height of box b

    # Compute intersection
    A_inter = max(0,min(prediction_box[0]+w_a,gt_box[0]+w_b)-max(prediction_box[0],gt_box[0]))*max(0,min(prediction_box[3]+h_a,gt_box[3]+h_b)-max(prediction_box[3],gt_box[3]))
...
def get_all_box_matches(prediction_boxes, gt_boxes, iou_threshold):
    ...
    p_boxes_return = np.zeros(len(prediction_boxes))
    gt_boxes_return = []
    iou_values = []

    for i in range(len(prediction_boxes)):
        for j in range(len(gt_boxes)):
            iou_val = calculate_iou(prediction_boxes[i],gt_boxes[j])
...
def calculate_precision_recall_all_images(
    ...
    for i in range(len(all_prediction_boxes)):
        tptemp,fptemp,fntemp = calculate_individual_image_result(all_prediction_boxes, all_gt_boxes, iou_threshold)
        tp += tptemp
        fp += fptemp
        fn += fntemp
...
def get_precision_recall_curve(
    ...
    for i in range(len(confidence_thresholds)):
        #np.where(confidence_scores[:][:]>confidence_thresholds[i])
        #condition = confidence_scores[:][1]> confidence_thresholds[i]
        print("conf:",confidence_scores[:][2])
        print("conf:",confidence_scores[:][10])
        #validboxes = confidence_thresholds[condition]
